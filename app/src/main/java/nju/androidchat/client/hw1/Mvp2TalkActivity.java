        ...
        runOnUiThread(() -> {
                    ...
                    for (ClientMessage message : messages) {
                        ...
                        if (text.matches(pattern)) {
                            String img = text.substring(text.indexOf('(') + 1, text.length() - 1);
                            if (message.getSenderUsername().equals(this.presenter.getUsername())) {
                                content.addView(new ItemImgSend(this, img, message.getMessageId(), this));
//                              content.addView(new ItemImgSend(this,"http://news.sciencenet.cn/upload/news/images/2011/3/20113301123128272.jpg",message.getMessageId(),this));
                                ...
                                content.addView(new ItemImgReceive(this, img, message.getMessageId()));
                            }
                        } else {
                            if (message.getSenderUsername().equals(this.presenter.getUsername())) {
